...
const circleStyle1 = {fill: "#FFCE63",}
const circleStyle2 = {fill: "#10316B",}
...
function Header() {
  ...
  useEffect(() => {
    ...
    // 헤더 스타일 설정
    if(path === "/") setHeaderStyle({background: "#F2F7FF", borderBottom: "none"});
    else setHeaderStyle({background: "white", borderBottom: "1px solid #ccc"});

    // progress bar 설정
    if(path === "/git") setStyleArr([{}, circleStyle1, circleStyle2, circleStyle2]);
    else if(path === "/info") setStyleArr([{}, circleStyle2, circleStyle1, circleStyle2]);
    else if(path === "/result") setStyleArr([{}, circleStyle2, circleStyle2, circleStyle1]);
    else setStyleArr([{display: "none"}, circleStyle2, circleStyle2, circleStyle1]);
    }, [history.location]);

    return (
      <header style={headerStyle}><div>
        <Link to="/"><div className={"logo"}>Gitppo</div></Link>

        {/* Progress bar */}
        <div className={"header-progressbar"} style={styleArr[0] || {display: "none"}}>
        ...
        <div className={"subtitle no-mobile"}>개발자의 취업 메이트, Gitppo</div>
      </div>
    </header>
  );
}
